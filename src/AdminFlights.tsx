...
export default function AdminFlights() {
  ...
  const load = async () => {
    ...
    let query = supabase
      .from('services')
      .select('id, title, origin, destination, departure_time, arrival_time, price, seats_available, photo_url, status, created_at, company_id, companies(business_name)')
      .eq('category', 'flight')
      ...
      <div style={{ padding: isDesktop ? '24px 28px' : '16px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: '14px', overflow: 'hidden', marginBottom: '14px' }}>
          <div style={{ height: '160px', background: viewing.photo_url ? undefined : `linear-gradient(135deg, #F97316, ${COLORS.primary})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {viewing.photo_url ? <img src={viewing.photo_url} alt={viewing.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Icon name="plane" size={34} color="white" />}
          </div>
          <div style={{ padding: '16px' }}>
            ...
            <div style={{ marginTop: '14px', background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
              ...
              <div style={{ flex: 1, background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
                <p style={{ fontSize: '10px', color: COLORS.textMuted, fontWeight: 700 }}>ARRIVAL</p>
                <p style={{ fontSize: '12.5px', fontWeight: 700, color: COLORS.text, marginTop: '2px' }}>{viewing.arrival_time ? new Date(viewing.arrival_time).toLocaleString() : 'Not set'}</p>
              </div>
            </div>
    ...
    <div style={{ padding: isDesktop ? '24px 28px' : '16px', maxWidth: '1400px', margin: '0 auto', overflowY: 'auto' as const, height: '100%' }}>
      ...
      {loading ? (
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: '14px', padding: '30px', textAlign: 'center' as const, color: COLORS.textMuted }}>
          <p style={{ fontSize: '13px' }}>No flight listings found.</p>
        </div>
      ) : (
          ...
          {listings.map((l, idx) => (
            <div key={l.id} onClick={() => setViewing(l)}
              ...
              <div style={{ width: '40px', height: '40px', borderRadius: '10px', overflow: 'hidden', flexShrink: 0, background: '#F97316', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {l.photo_url ? <img src={l.photo_url} alt={l.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Icon name="plane" size={18} color="white" />}
              </div>
