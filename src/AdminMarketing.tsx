...
const EMPTY_FORM = { title: '', message: '', image_url: '', link_url: '' }
...
export default function AdminMarketing() {
  ...
  async function save() {
    if (!form.title.trim() || !form.message.trim()) {
      setError('Title da message dole ne.')
      return
    }
    setSaving(true)
    setError('')
  ...
  return (
    <div style={{ padding: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
        <p style={{ fontSize: '11.5px', fontWeight: 700, color: COLORS.textMuted, letterSpacing: '0.4px' }}>
          HOME BANNERS
        </p>
        <div
          onClick={openNew}
          style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 12px', borderRadius: '8px', background: COLORS.primary, cursor: 'pointer' }}
        >
            ...
            <div key={b.id} style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: '14px', padding: '14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <span style={{
                  ...
                  color: b.active ? COLORS.green : COLORS.textMuted,
                  background: b.active ? '#F0FDF4' : '#F1F5F9',
              ...
              {b.image_url && (
                <img src={b.image_url} alt="" style={{ width: '100%', height: '90px', objectFit: 'cover', borderRadius: '10px', marginTop: '8px' }} />
              )}
              <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
                <div onClick={() => openEdit(b)} style={{ flex: 1, textAlign: 'center' as const, padding: '8px', borderRadius: '8px', border: `1px solid ${COLORS.border}`, fontSize: '12px', fontWeight: 600, color: COLORS.text, cursor: 'pointer' }}>
                  Edit
                </div>
                <div onClick={() => toggleActive(b)} style={{ flex: 1, textAlign: 'center' as const, padding: '8px', borderRadius: '8px', border: `1px solid ${COLORS.border}`, fontSize: '12px', fontWeight: 600, color: COLORS.text, cursor: 'pointer' }}>
                  {b.active ? 'Hide' : 'Activate'}
                </div>
                <div onClick={() => remove(b)} style={{ flex: 1, textAlign: 'center' as const, padding: '8px', borderRadius: '8px', border: `1px solid ${COLORS.red}`, fontSize: '12px', fontWeight: 600, color: COLORS.red, cursor: 'pointer' }}>
                  Delete
                </div>
              </div>
            </div>
      ...
      {showForm && (
        ...
        >
          ...
          >
            <p style={{ fontSize: '15px', fontWeight: 800, color: COLORS.text, marginBottom: '12px' }}>
            ...
            <textarea
              ...
              placeholder="Message"
              style={{ width: '100%', padding: '10px', borderRadius: '10px', border: `1px solid ${COLORS.border}`, fontSize: '13px', minHeight: '64px', marginBottom: '10px', color: COLORS.text }}
            />
            <input
            ...
            <input
              ...
              placeholder="Link URL (optional, where tapping the banner goes)"
              style={{ width: '100%', padding: '10px', borderRadius: '10px', border: `1px solid ${COLORS.border}`, fontSize: '13px', color: COLORS.text }}
            />
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '14px' }}>
              <div
                onClick={() => !saving && save()}
                style={{ flex: 1, textAlign: 'center' as const, padding: '11px', borderRadius: '10px', background: COLORS.primary, color: '#fff', fontWeight: 700, fontSize: '13.5px', cursor: 'pointer' }}
              >
                {saving ? '...' : 'Save'}
              </div>
              <div
