...
function Tours() {
  ...
  let visible = typeFilter ? tours.filter((t) => t.tour_type === typeFilter) : tours
  ...
  const lowestPriceId = tours.length > 1 ? [...tours].sort((a, b) => a.price - b.price)[0].id : null
  const topRatedId = (() => {
    const rated = tours.filter((h) => h.avgRating !== null)
                ...
                <div style={{ padding: '14px' }}>
                  ...
                  {t.avgRating !== null && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '8px' }}>
                      <span style={{ background: '#DCFCE7', color: COLORS.green, fontSize: '11.5px', fontWeight: 800, padding: '2px 7px', borderRadius: '6px' }}>{t.avgRating.toFixed(1)}</span>
                      <span style={{ fontSize: '11.5px', color: COLORS.textMuted }}>({t.reviewCount} review{t.reviewCount !== 1 ? 's' : ''})</span>
                    </div>
                  )}
