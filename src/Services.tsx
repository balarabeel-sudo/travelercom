...
const CATEGORY_META: Record<string, { label: string; icon: string; unitLabel: string; isTransport: boolean }> = {
  bus: { label: 'Bus Stations', icon: '🚌', unitLabel: 'seats', isTransport: true },
  train: { label: 'Railway', icon: '🚆', unitLabel: 'seats', isTransport: true },
  flight: { label: 'Domestic Flights', icon: '✈️', unitLabel: 'seats', isTransport: true },
  tour: { label: 'Tours & Attractions', icon: '🗺️', unitLabel: 'slots', isTransport: false },
  ...
  return (
    <div style={{ minHeight: '100vh', background: COLORS.bg, maxWidth: '480px', margin: '0 auto', paddingBottom: '40px' }}>
      ...
      <div style={{
        ...
        <h1 style={{ fontSize: '17px', fontWeight: 800, color: COLORS.text }}>{meta.icon} {meta.label}</h1>
      </div>
      ...
      <div style={{ padding: '16px' }}>
        <div style={{
          ...
          <input
            type="text"
            placeholder={meta.isTransport ? 'Destination (e.g. Abuja)' : 'City / Location'}
            value={city}
        ...
        {loading ? (
          ...
          <div style={{ background: COLORS.card, borderRadius: '16px', padding: '30px', textAlign: 'center', color: COLORS.textMuted, boxShadow: '0 2px 10px rgba(0,0,0,0.06)' }}>
            <p style={{ fontSize: '13px' }}>No {meta.label.toLowerCase()} found</p>
                ...
                <div style={{ display: 'flex', gap: '12px', marginBottom: '10px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ fontSize: '12px', color: COLORS.textMuted }}>
                      {meta.isTransport && it.origin ? `${it.origin} → ${it.destination}` : it.destination}
                    </p>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    ...
                    {it.seats_available !== null && (
                      <p style={{ fontSize: '11px', color: COLORS.textMuted }}>{it.seats_available} {meta.unitLabel} available</p>
                    )}
                  </div>
                  <button
