...
export default function AdminTransport() {
  ...
  const load = async () => {
    ...
    let query = supabase
      .from('services')
      .select('id, title, category, origin, destination, departure_time, price, seats_available, photo_url, status, created_at, company_id, companies(business_name)')
      .in('category', typeFilter === 'all' ? ['bus', 'train'] : [typeFilter])
      ...
      <div style={{ padding: isDesktop ? '24px 28px' : '16px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: '14px', overflow: 'hidden', marginBottom: '14px' }}>
          <div style={{ height: '160px', background: viewing.photo_url ? undefined : `linear-gradient(135deg, #F97316, ${COLORS.primary})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {viewing.photo_url ? <img src={viewing.photo_url} alt={viewing.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Icon name="bus" size={34} color="white" />}
          </div>
          <div style={{ padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
              <div>
                <p style={{ fontSize: '15px', fontWeight: 800, color: COLORS.text }}>{viewing.title}</p>
                <p style={{ fontSize: '12px', color: COLORS.textMuted, marginTop: '2px' }}>{viewing.companies?.business_name || 'Unknown company'} · {viewing.category === 'train' ? 'Train' : 'Bus'}</p>
              </div>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '14px' }}>
              <div style={{ flex: 1, background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
                <p style={{ fontSize: '10px', color: COLORS.textMuted, fontWeight: 700 }}>ROUTE</p>
                <p style={{ fontSize: '12.5px', fontWeight: 700, color: COLORS.text, marginTop: '2px' }}>{viewing.origin ? `${viewing.origin} → ${viewing.destination}` : (viewing.destination || 'Not set')}</p>
              </div>
              <div style={{ flex: 1, background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
                <p style={{ fontSize: '10px', color: COLORS.textMuted, fontWeight: 700 }}>PRICE / SEAT</p>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
              <div style={{ flex: 1, background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
                <p style={{ fontSize: '10px', color: COLORS.textMuted, fontWeight: 700 }}>DEPARTURE</p>
                <p style={{ fontSize: '12.5px', fontWeight: 700, color: COLORS.text, marginTop: '2px' }}>{viewing.departure_time ? new Date(viewing.departure_time).toLocaleString() : 'Not set'}</p>
              </div>
              <div style={{ flex: 1, background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
            ...
            <div style={{ marginTop: '8px', background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
  ...
  return (
    <div style={{ padding: isDesktop ? '24px 28px' : '16px', maxWidth: '1400px', margin: '0 auto', overflowY: 'auto' as const, height: '100%' }}>
      <div style={{ marginBottom: '4px' }}>
        <h2 style={{ fontSize: isDesktop ? '20px' : '17px', fontWeight: 800, color: COLORS.text }}>Transport</h2>
        <p style={{ fontSize: '12px', color: COLORS.textMuted, marginTop: '2px' }}>All bus and train listings across the platform.</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '7px', overflowX: 'auto' as const, marginBottom: '8px', paddingBottom: '2px' }}>
        {TYPE_FILTERS.map((f) => (
          <button key={f.key} onClick={() => setTypeFilter(f.key)}
            style={{ whiteSpace: 'nowrap' as const, padding: '7px 13px', borderRadius: '999px', border: `1px solid ${typeFilter === f.key ? COLORS.primary : COLORS.border}`, background: typeFilter === f.key ? COLORS.primary : COLORS.card, color: typeFilter === f.key ? 'white' : COLORS.textMuted, fontSize: '11.5px', fontWeight: 700, cursor: 'pointer' }}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: '14px', padding: '30px', textAlign: 'center' as const, color: COLORS.textMuted }}>
          <p style={{ fontSize: '13px' }}>No transport listings found.</p>
        </div>
      ) : (
          ...
          {listings.map((l, idx) => (
            <div key={l.id} onClick={() => setViewing(l)}
              ...
              <div style={{ width: '40px', height: '40px', borderRadius: '10px', overflow: 'hidden', flexShrink: 0, background: '#F97316', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {l.photo_url ? <img src={l.photo_url} alt={l.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Icon name="bus" size={18} color="white" />}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: '13px', fontWeight: 700, color: COLORS.text }}>{l.title}</p>
                <p style={{ fontSize: '11px', color: COLORS.textMuted, marginTop: '1px' }}>{l.companies?.business_name || 'Unknown'} · {l.origin ? `${l.origin} → ${l.destination}` : (l.destination || 'No route set')}</p>
                <div style={{ display: 'flex', gap: '10px', marginTop: '5px' }}>
                  <span style={{ fontSize: '11px', color: COLORS.textMuted }}>{formatNaira(l.price)}/seat</span>
