...
export default function AdminEventCenters() {
  ...
  const load = async () => {
    ...
    let query = supabase
      .from('services')
      .select('id, title, destination, price, seats_available, photo_url, status, event_type, created_at, company_id, companies(business_name)')
      ...
      <div style={{ padding: isDesktop ? '24px 28px' : '16px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: '14px', overflow: 'hidden', marginBottom: '14px' }}>
          <div style={{ height: '160px', background: viewing.photo_url ? undefined : `linear-gradient(135deg, #F97316, ${COLORS.primary})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {viewing.photo_url ? <img src={viewing.photo_url} alt={viewing.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Icon name="tent" size={34} color="white" />}
          </div>
          <div style={{ padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
              <div>
                <p style={{ fontSize: '15px', fontWeight: 800, color: COLORS.text }}>{viewing.title}</p>
                <p style={{ fontSize: '12px', color: COLORS.textMuted, marginTop: '2px' }}>{viewing.companies?.business_name || 'Unknown company'}{viewing.event_type ? ` · ${viewing.event_type}` : ''}</p>
              </div>
  ...
  return (
    <div style={{ padding: isDesktop ? '24px 28px' : '16px', maxWidth: '1400px', margin: '0 auto', overflowY: 'auto' as const, height: '100%' }}>
      <div style={{ marginBottom: '4px' }}>
        <h2 style={{ fontSize: isDesktop ? '20px' : '17px', fontWeight: 800, color: COLORS.text }}>Event Centers</h2>
        <p style={{ fontSize: '12px', color: COLORS.textMuted, marginTop: '2px' }}>All event center listings across the platform.</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', margin: '14px 0' }}>
        <input
          ...
          placeholder="Search event center name..."
      ...
      {loading ? (
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: '14px', padding: '30px', textAlign: 'center' as const, color: COLORS.textMuted }}>
          <p style={{ fontSize: '13px' }}>No event center listings found.</p>
        </div>
      ) : (
          ...
          {listings.map((l, idx) => (
            <div key={l.id} onClick={() => setViewing(l)}
              ...
              <div style={{ width: '40px', height: '40px', borderRadius: '10px', overflow: 'hidden', flexShrink: 0, background: '#F97316', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {l.photo_url ? <img src={l.photo_url} alt={l.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Icon name="tent" size={18} color="white" />}
              </div>
