...
export default function AdminVehicleRentals() {
  ...
  const load = async () => {
    ...
    let query = supabase
      .from('services')
      .select('id, title, destination, brand, model, year, transmission, rental_mode, price, seats_available, photo_url, status, created_at, company_id, companies(business_name)')
      ...
      <div style={{ padding: isDesktop ? '24px 28px' : '16px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: '14px', overflow: 'hidden', marginBottom: '14px' }}>
          <div style={{ height: '160px', background: viewing.photo_url ? undefined : `linear-gradient(135deg, #F97316, ${COLORS.primary})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {viewing.photo_url ? <img src={viewing.photo_url} alt={viewing.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Icon name="car" size={34} color="white" />}
          </div>
          <div style={{ padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
              <div>
                <p style={{ fontSize: '15px', fontWeight: 800, color: COLORS.text }}>{viewing.title}</p>
                <p style={{ fontSize: '12px', color: COLORS.textMuted, marginTop: '2px' }}>{viewing.companies?.business_name || 'Unknown company'}</p>
              </div>
              <StatusBadge status={viewing.status} />
            </div>

            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' as const, marginTop: '14px' }}>
              {viewing.transmission && <Tag text={viewing.transmission} />}
              {viewing.year != null && <Tag text={String(viewing.year)} />}
              {viewing.rental_mode && <Tag text={RENTAL_MODE_LABEL[viewing.rental_mode] || viewing.rental_mode} />}
            </div>

            <div style={{ display: 'flex', gap: '8px', marginTop: '10px' }}>
              <div style={{ flex: 1, background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
                <p style={{ fontSize: '10px', color: COLORS.textMuted, fontWeight: 700 }}>CITY</p>
                <p style={{ fontSize: '12.5px', fontWeight: 700, color: COLORS.text, marginTop: '2px' }}>{viewing.destination || 'Not set'}</p>
              </div>
              <div style={{ flex: 1, background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
                <p style={{ fontSize: '10px', color: COLORS.textMuted, fontWeight: 700 }}>PRICE / DAY</p>
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
              <div style={{ flex: 1, background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
                <p style={{ fontSize: '10px', color: COLORS.textMuted, fontWeight: 700 }}>FLEET AVAILABLE</p>
                <p style={{ fontSize: '12.5px', fontWeight: 700, color: COLORS.text, marginTop: '2px' }}>{viewing.seats_available ?? '—'}</p>
              </div>
              <div style={{ flex: 1, background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
            ...
            <div style={{ marginTop: '8px' }}>
              <div style={{ background: COLORS.bg, borderRadius: '10px', padding: '10px' }}>
  ...
  return (
    <div style={{ padding: isDesktop ? '24px 28px' : '16px', maxWidth: '1400px', margin: '0 auto', overflowY: 'auto' as const, height: '100%' }}>
      <div style={{ marginBottom: '4px' }}>
        <h2 style={{ fontSize: isDesktop ? '20px' : '17px', fontWeight: 800, color: COLORS.text }}>Vehicle Rental</h2>
        <p style={{ fontSize: '12px', color: COLORS.textMuted, marginTop: '2px' }}>All rental vehicle listings across the platform.</p>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: '14px', padding: '30px', textAlign: 'center' as const, color: COLORS.textMuted }}>
          <p style={{ fontSize: '13px' }}>No vehicle rental listings found.</p>
        </div>
      ) : (
          ...
          {listings.map((l, idx) => (
            <div key={l.id} onClick={() => setViewing(l)}
              ...
              <div style={{ width: '40px', height: '40px', borderRadius: '10px', overflow: 'hidden', flexShrink: 0, background: '#F97316', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {l.photo_url ? <img src={l.photo_url} alt={l.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <Icon name="car" size={18} color="white" />}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: '13px', fontWeight: 700, color: COLORS.text }}>{l.title}</p>
                <p style={{ fontSize: '11px', color: COLORS.textMuted, marginTop: '1px' }}>{l.companies?.business_name || 'Unknown'} · {l.destination || 'No city'}</p>
                <div style={{ display: 'flex', gap: '10px', marginTop: '5px', flexWrap: 'wrap' as const }}>
                  <span style={{ fontSize: '11px', color: COLORS.textMuted }}>{formatNaira(l.price)}/day</span>
                  <span style={{ fontSize: '11px', color: COLORS.textMuted }}>{(l.bookings_count ?? 0).toLocaleString()} Bookings</span>
                  {l.rental_mode && <span style={{ fontSize: '11px', color: COLORS.primary, fontWeight: 700 }}>{RENTAL_MODE_LABEL[l.rental_mode] || l.rental_mode}</span>}
                </div>
              </div>
...
function Tag({ text }: { text: string }) {
  return (
    <span style={{ fontSize: '10.5px', fontWeight: 700, padding: '4px 10px', borderRadius: '999px', background: COLORS.bg, color: COLORS.textMuted, border: `1px solid ${COLORS.border}` }}>
      {text}
    </span>
  )
}
