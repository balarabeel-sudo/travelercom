...
function AddTourListing() {
      ...
      if (savedListing) {
        ...
        if (userData?.user) {
          await supabase.rpc('log_audit', {
            ...
            p_previous: null,
            p_new: { title: title.trim(), category: 'tour' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: COLORS.bg, maxWidth: '480px', margin: '0 auto', paddingBottom: '40px' }}>
      <div style={{ padding: '18px 20px', display: 'flex', alignItems: 'center', gap: '12px', background: COLORS.card, boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
        <span onClick={() => navigate('/home')} style={{ cursor: 'pointer', display: 'flex' }}><Icon name="arrowLeft" size={20} color={COLORS.text} /></span>
        <h1 style={{ fontSize: '17px', fontWeight: 800, color: COLORS.text }}>{editId ? 'Edit Tour Listing' : 'Add Tour Listing'}</h1>
      </div>
      ...
      <div style={{ padding: '20px 16px' }}>
        <div style={{ background: COLORS.card, borderRadius: '10px', padding: '10px 14px', marginBottom: '16px', display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: 700, color: COLORS.primary }}>
          <Icon name="map" size={14} color={COLORS.primary} /> Tour
        </div>
        ...
        <div style={{ background: COLORS.card, borderRadius: '14px', padding: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)' }}>
              ...
              {TOUR_TYPES.map((t) => {
                ...
                return (
                  <span
                    ...
                    <Icon name={TOUR_TYPE_ICON[t]} size={13} color={active ? 'white' : COLORS.textMuted} /> {t}
                  </span>
                )
              })}
            </div>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <div style={{ flex: 1 }}>
              <Field label="Gate Fee (₦, optional)">
                <input type="number" placeholder="e.g. 500" value={gateFee} onChange={(e) => setGateFee(e.target.value)} style={inputStyle} />
              </Field>
            </div>
            <div style={{ flex: 1 }}>
              <Field label="Vehicle Fee (₦, optional)">
                <input type="number" placeholder="e.g. 1000" value={vehicleFee} onChange={(e) => setVehicleFee(e.target.value)} style={inputStyle} />
              </Field>
            </div>
          </div>
